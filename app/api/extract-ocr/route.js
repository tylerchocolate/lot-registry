...
If lbs shown convert to kg (1 lb = 0.453592 kg) and note it.
...
confidence must be "high", "medium", or "unreadable".`,
    user: 'Read the weight on each scale photo and return individual readings and total.',
  },
  ferment: {
    prefix: 'ferment_',
...
export async function POST(request) {
  ...
  for (const [key, prompt] of Object.entries(PROMPTS)) {
    try {
      // Query storage.objects directly - works because server client has service role
      // Path pattern: {user_uuid}/association/{step_prefix}{step_num}/{timestamp}.jpg
      const { data: objects, error: objError } = await db
        .schema('storage')
        .from('objects')
        .select('name')
        .eq('bucket_id', 'photos')
        .ilike('name', `%/association/${prompt.prefix}%`)
        .order('created_at', { ascending: true })
        .limit(10);

      if (objError) {
        results[key] = { error: 'storage_query_failed', detail: objError.message };
        continue;
      }
      ...
      const urls = objects.map((o) => `${STORAGE_BASE}/${o.name}`);
