...
export async function getLots() {
  ...
  const { data, error } = await db.from('lots').select(`
    ...
    ocr_weight_kg, ocr_weight_confidence, ocr_max_temp_celsius, ocr_temp_confidence,
    ocr_moisture_percent, ocr_moisture_confidence, ocr_extracted_at
...
export async function getLot(id) {
  ...
  const { data, error } = await db.from('lots').select(`
    ...
    producer, net_weight, gross_weight, bag_count, bag_count_reg, bag_weight,
    harvest_date, ferment_start_at, ferment_end_at, ferment_days, turns,
    fermentor_type, ferment_notes, dry_start_at, dry_end_at, dry_days,
    final_moisture, dry_method, dry_notes,
    buyer, dispatch_date, transport_ref, observations, farm_ica, qr_url,
    quality_check_at, varieties, cut_rows, cut_blend,
    ...
    ocr_extracted_at, created_at, updated_at
  `).eq('id', id).single();
  if (error) throw error;
  return data;
}
